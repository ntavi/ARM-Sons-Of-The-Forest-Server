...
#!/bin/sh
WINEPREFIX=\${WINEPREFIX:-~/.wine} WINEARCH=win32 box86 /opt/wine/bin/wine \$@
EOF
cat << EOF > /usr/local/bin/wine64
#!/bin/sh
WINEPREFIX=\${WINEPREFIX:-~/.wine64} WINEARCH=win64 box64 /opt/wine/bin/wine64 \$@
EOF
cat << EOF > /usr/local/bin/wineserver
#!/bin/sh
WINEPREFIX=\${WINEPREFIX:-~/.wine64} WINEARCH=win64 box64 /opt/wine/bin/wineserver \$@
EOF
